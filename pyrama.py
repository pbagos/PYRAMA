...
def gwas_meta_analysis(input_files, output_file, inheritance_model, effect_size_type,
                       ...
                       imputation=False, bayesian_meta='NO', r2threshold=None,
                       population=None, maf=None, ref=None, imp_list=None):
    print("Running GWAS Meta-Analysis:")

    # Column definitions for each case
    ...
    # Peek header of the first file to detect case2 without merging
    try:
    ...
    # Handle Case 2: BETA and SE inputs should be passed directly to the external script
    if all(col in sample_header for col in case_2_columns):
        print("Detected  BETA and SE in the input files ")

        # Imputation logic remains for SNP-level imputation, results used as single file input
        if imputation:
            ...
            # Merge only for imputation step
            ...
            script_inputs = "results/imputation_results.txt"
        else:
            # Directly use all input files for meta-analysis script
            script_inputs = ' '.join(input_files)

        print("Running Meta-Analysis with BETA and SE...")
        start_time = time.time()
        cmd = f"./pyrama_beta_se_meta {script_inputs} > {output_file}"
        os.system(cmd)
    ...
    # For all other cases, merge the input files first
    merged_df = merge_input_files(input_files)
    data_df = merged_df

    # Bivariate two-study case with allele counts
    if all(col in data_df.columns for col in case_12_columns):
        print("Bivariate meta-analysis (allele counts detected)")
        data_subset = data_df[case_12_columns]
        result = bivariate.biv_meta_analysis(data_subset, inheritance_model,
                                             effect_size_type, robust_method,
                                             type_of_effect, approximate_max)
        result.to_csv(output_file, sep='\t', index=False)

    # Bivariate GWAS with BETA and SE from two studies
    ...
    # Case 1: Discrete phenotype, allele counts
        ...
        if robust_method == 'FAST':
            ...
            result = bayesian.meta_analysis(data_subset, inheritance_model,
                                            effect_size_type, robust_method,
                                            approximate_max)
        else:
            print("Standard meta-analysis")
            result = meta_analysis.meta_analysis(data_subset, inheritance_model,
                                                 effect_size_type, robust_method,
                                                 type_of_effect, approximate_max)
        result.to_csv(output_file, sep='\t', index=False)

    # Case 3: Continuous phenotype, means and sds
        ...
        result = cont_meta_analysis.meta_analysis(data_subset, inheritance_model,
                                                  robust_method, type_of_effect)
        result.to_csv(output_file, sep='\t', index=False)
...
| ##         | ##    | ##  | ##| ##  | ##| ##  \# | ##| ##  | ##
 \##          \##     \##   \## \##   \## \##      \## \##   \##
    ...
    parser.add_argument("--i", "--input", "--input_file", nargs='+', required=True,
                        help="Paths to input data files (space-separated).")
    parser.add_argument("--o", "--output", "--output_file", required=True,
                        help="Path to save the output results.")
    parser.add_argument("--inheritance_model", required=False,
                        help="Inheritance model to use. ADDITIVE, RECESSIVE or DOMINANT")
    parser.add_argument("--effect_size_type", required=False,
                        help="Type of effect size. OR or CATT")
    parser.add_argument("--robust_method", required=False,
                        help="Robust method to use. MIN, MAX or MERT, or FAST")
    parser.add_argument("--type_of_effect", required=False,
                        help="Type of effect. FIXED or RANDOM")
    parser.add_argument("--approximate_max", required=False,
                        help="Approximate maximum. YES or NO")
    parser.add_argument("--biv_ma", default="NO",
                        help="Bivariate meta-analysis (default: NO).")

    parser.add_argument("--imputation", action="store_true",
                        help="Enable imputation step (default: disabled).")
    parser.add_argument("--bayesian_meta", default="NO",
                        help="Bayesian meta-analysis (default: NO).")
    parser.add_argument("--r2threshold", required=False,
                        help="R2 threshold for imputation.")
    parser.add_argument("--population", required=False,
                        help="Population for imputation.")
    parser.add_argument("--maf", required=False,
                        help="Minor allele frequency for imputation.")
    parser.add_argument("--ref", required=False,
                        help="Reference panel for imputation.")
    parser.add_argument("--imp_list", required=False,
                        help="List of SNPs to impute (optional).")

    args = parser.parse_args()

    gwas_meta_analysis(args.i, args.o, args.inheritance_model,
                       args.effect_size_type, args.robust_method,
                       args.type_of_effect, args.approximate_max,
                       args.biv_ma, args.imputation,
                       args.bayesian_meta, args.r2threshold,
                       args.population, args.maf, args.ref,
                       args.imp_list)
